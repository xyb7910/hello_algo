...
对于 $DFS$ 搜索数，让每个节点存储：考虑第 $u$ 只猫是否放到第 $k$ 辆车上。
- 放到第 $k$ 辆车上，那么第 $k$ 辆车的总重量加上 第 $u$ 只猫的重量
- 不放到第 $k$ 辆车上，那么应该新增加一辆车去放第 $u$ 只小猫。
对于**最优性性剪枝**，可以考虑：如果车辆的总数量 $\ge$ 猫的数量的时候必然要剪枝。 
对于**可行性剪枝**，当第 $k$ 辆车的总量 $<$ 最大限制的时候，才可以放第 $u$ 只猫。 
为了让搜索树的高度最小，可以每次让**最重的猫先上车**，这样确保当前子树的高度最小。
